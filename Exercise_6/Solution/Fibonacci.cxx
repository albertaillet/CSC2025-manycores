...
int main(int argc, char* argv[]) {
    ...
    benchmark("Iterative algorithm", fibo, fibo, [N](const auto & input, auto & output){
      ...
      while (output.size() < N) {
        ...
        output.push_back(input[size-2] + input[size-1]);
      }
    });
    print(fibo);
    ...
    //   A: In an inclusive scan, the output at location i contains the result of applying the operator to input element i,
    //      for an exclusive scan, element i is not part of the sequence.
    // - Why is this algorithm slower than the trivial sequence from task 1?
    //   A: It is doing a lot more work, because two numbers are passed through the sequence.
    benchmark("Sequential matrix algorithm", fiboInputs, fiboResults, [](auto const & input, auto & output){
        std::exclusive_scan(input.begin(), input.end(),
                            output.begin(), FiboMatrix{1,0,0,1}, std::multiplies<>{});
    });
    print(fiboResults);
    ...
    // A: In the best case, it would be N / log(N), but partitioning the range and assigning work to threads will reduce
    //    the speedup considerably. With more threads, smaller partitions are used, and more work has to be done in total.
    //    Therefore, a linear scaling with the number of threads is not possible.
